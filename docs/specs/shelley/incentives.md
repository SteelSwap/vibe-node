...
::: landscape
![Positioning of this Deliverable (outlined in red).](d4-depends.pdf)
:::

***Note: this document is subject to change. In particular, it may be necessary to include a simplified fees calculation in the incentivised TestNet, where all the fees accrue to the Treasury. Also note that some terms and symbols that were used in SL-D1 have been changed in this document in order to simplify the explanations that are given here. These are outlined in Appendix [4](#app:diffs){reference-type="ref" reference="app:diffs"}.***
...
### Operator-Set Parameters {#operator-set-parameters .unnumbered}
...
### Private Stake Pools {#private-stake-pools .unnumbered}
...
### Sources of Rewards {#sources-of-rewards .unnumbered}

Rewards are taken from three sources:

1.  the monetary expansion distribution (Section [3.1](#sec:expansion){reference-type="ref" reference="sec:expansion"});

2.  transaction fees (Section [3.2](#sec:fees){reference-type="ref" reference="sec:fees"});

3.  non-refundable deposits.

In the Incentivised TestNet, only the first source is likely to be used.

### The Rewards that are received by a StakePool {#the-rewards-that-are-received-by-a-stakepool .unnumbered}

The rewards that are received by a StakePool will be proportional to the total amount of ADA that it controls as a fraction of the total ADA that is in circulation. The *treasury top slice* is deducted from the total *rewards* for the epoch, then the remainder is distributed proportionately to each StakePool, depending on the stake it controls. In order to avoid domination by large StakePools, in the MainNet, a rewards *cap* will be applied to any single pool, based on the target number of StakePools, $N^{\textit{Pools}}$. This is described in more detail in Section [3](#sec:MainNet){reference-type="ref" reference="sec:MainNet"}.
...
### The Rewards that are given to Owner(s) and Delegators {#the-rewards-that-are-given-to-owners-and-delegators .unnumbered}
...
# The Simplified Incentives Scheme (Shelley Incentivised TestNet) {#sec:TestNet}
...
### Transaction Fees in the Incentivised TestNet (possible addition) {#transaction-fees-in-the-incentivised-testnet-possible-addition .unnumbered}
...
### The Rewards that are received by each Delegator per Epoch {#the-rewards-that-are-received-by-each-delegator-per-epoch .unnumbered}
...
## Calculating Rewards in External Currencies {#sec:conversion}
...
![image](RCT1.pdf){height="6in"}

![image](RCT2.pdf){width="120%"}
...
### StakePool Parameters {#stakepool-parameters .unnumbered}

![image](RCT3.pdf){width="120%"}
...
### Calculated Rewards {#calculated-rewards .unnumbered}

::: minipage
![image](RCT4.pdf){width="120%"}

![image](RCT5.pdf){width="120%"}
:::
...
## How Rewards are returned to Owners and Delegators {#sec:rewards-return}
...
# The Full Incentives Scheme (Shelley MainNet Implementation) {#sec:MainNet}
...
::: center
  ...
  **$\textit{MER}_E$**        **The "Monetary Expansion Rate" for Epoch $E$**.\
                              ***See Section [3.1](#sec:expansion){reference-type="ref" reference="sec:expansion"}.***
  ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
:::
...
| **$\textit{Deposits}_E$**        |                            | **The non-refundable deposits for Epoch $E$**       |                                                                                                   |
+----------------------------------+----------------------------+-----------------------------------------------------+---------------------------------------------------------------------------------------------------+
| $\textit{MER}_E$                 | 10%-15%                    | The "Monetary Expansion Rate" for Epoch $E$         | *See Section [3.1](#sec:expansion){reference-type="ref" reference="sec:expansion"}.*              |
+----------------------------------+----------------------------+-----------------------------------------------------+---------------------------------------------------------------------------------------------------+
...
## The Monetary Expansion Rate {#sec:expansion}

::::: {.figure latex-placement="h!"}
::: center
![image](AdaPerYear.pdf){width="75%"}\
![image](Rewards.pdf){width="75%"}
:::
...
### The Rewards that are received by each Delegator for Epoch $E$ {#the-rewards-that-are-received-by-each-delegator-for-epoch-e .unnumbered}
...
### The Rewards that are received by each StakePool Owner for Epoch $E$ {#the-rewards-that-are-received-by-each-stakepool-owner-for-epoch-e .unnumbered}
...
::: center
  **Parameter**        **Definition**                                                                                                                                                                                                                                                                **Calculated as**
  ...
  $O^{Rewards}$        The ADA rewards that are received by $O$. This may be capped if the owner fraction is too large, as described in Section [\[sec:penalties\]](#sec:penalties){reference-type="ref" reference="sec:penalties"}, or reduced to zero if any owner fails to honour their pledge.   $R^{\textit{Net}} \times O^\%$
:::
...
### StakePool Rewards Penalties {#sec:penalties .unnumbered}
...
## Calculating Rewards in External Currencies {#calculating-rewards-in-external-currencies}

Rewards can be converted to other currencies using the current exchange rate, exactly as described in Section [2.2](#sec:conversion){reference-type="ref" reference="sec:conversion"}. Realising these funds will require the use of an external *exchange* service.
...
### Key Parameters {#key-parameters-1 .unnumbered}
...
### Circulation and Distribution {#circulation-and-distribution .unnumbered}

![image](RCM2.pdf){width="160%"}
...
### StakePool Parameters {#stakepool-parameters-1 .unnumbered}

![image](RCM3.pdf){width="125%"}
...
## Summary of Differences between the Two Incentives Schemes {#sec:summary}
...
# Summary of Differences in terminology etc. to [@delegation_design] {#app:diffs}
