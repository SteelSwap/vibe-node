...
- If a transaction is accepted and marked as paying fees only (i.e. $\fun{txvaltag}\, tx \in \Yes$), then the only change to the ledger when processing the transaction is that the inputs marked for paying fees are moved to the fee pot.
...
- A valid transaction that does not forge tokens satisfies the accounting property of the Shelley ledger where the type $\Coin$ is replaced by $\Value$.
