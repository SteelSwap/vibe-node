...
*Proof.* The property (\[prop:minimal-refund\]) (p. ) for the minimal refund can be proven automatically via
...
*Proof.* For the property (\[prop:reward-splitting\]) (p. ) for reward splitting we actually show a stronger one, by removing the floor function. Using the fractional values we get an upper bound for the real value and showing that this upper bound is bounded by $\hat{f}$ we show that the real value is also bounded by $\hat{f}$. To eliminate the sum, we use the identity $\frac{s +
