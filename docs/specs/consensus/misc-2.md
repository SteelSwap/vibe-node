...
### Invalid states
In a way, it is somewhat strange to have the hard fork mechanism be part of the Byron (\[byron:hardfork\]{reference-type="ref+label" reference="byron:hardfork"}) or Shelley ledger (\[shelley:hardfork\]{reference-type="ref+label" reference="shelley:hardfork"}) itself, rather than some overarching ledger on top. For Byron, a Byron ledger state where the *major* version is the (predetermined) moment of the hard fork is basically an invalid state, used only once to translate to a Shelley ledger. Similar, the *hard fork* part of the Shelley protocol version will never increase during Shelley's lifetime; the moment it *does* increase, that Shelley state will be translated to the (initial) state of the post-Shelley ledger.
...
We cannot forecast directly from the old ledger state to the new era either: this would result in a ledger view from the old era in the new era, violating the invariant we discussed in 1.1.1{reference-type="ref+label" reference="hfc:ledger:invalid-states"}.
