...
## Introduction {#time:introduction}
...
## Slots, blocks and stability {#time:slots-vs-blocks}

In [\[consensus:overview:k\]](#consensus:overview:k){reference-type="ref+label" reference="consensus:overview:k"} we discussed the fundamental parameter $k$: blocks that are more than $k$ blocks away from the tip of the chain are considered to be immutable by the consensus layer and no longer subject to rollback. We say that such blocks are *stable*.
...
The ledger layer however does not use block numbers to determine stability, but uses slot numbers to approximate it instead. This ultimately comes from the fact that in Ouroboros the length of an *epoch* is based on slots, not blocks, although this is something we may wish to revisit ([\[future:block-vs-slot\]](#future:block-vs-slot){reference-type="ref+label" reference="future:block-vs-slot"}).
...
As we saw in [1.1](#time:introduction){reference-type="ref+label" reference="time:introduction"}, we cannot do time conversions independent of a ledger state. This motivates the following definition:
...
Note that monotonicity ([\[time-conversion-monotone\]](#time-conversion-monotone){reference-type="ref+label" reference="time-conversion-monotone"}) should still apply.
...
## Ledger restrictions {#time:ledgerrestrictions}

### Era transitions must be stable

Monotonicity ([\[time-conversion-monotone\]](#time-conversion-monotone){reference-type="ref+label" reference="time-conversion-monotone"}) only talks about a chain's linear history; since the consensus layer needs to deal with rollbacks (switching to alternative chains) too, we will actually need a stronger property. Clearly, time conversions cannot be invariant under switching to arbitrary chains; after all, alternative chains might have era transitions in different places. The consensus layer however does not *support* switching to arbitrary alternative chains; we have a maximum rollback ([\[consensus:overview:k\]](#consensus:overview:k){reference-type="ref+label" reference="consensus:overview:k"}), and we never switch to a shorter chain ([\[consensus:overview:chainsel\]](#consensus:overview:chainsel){reference-type="ref+label" reference="consensus:overview:chainsel"}, [\[never-shrink\]](#never-shrink){reference-type="ref+label" reference="never-shrink"}). This means that we can model switching to an alternative chain as $$\mathtt{switch_{(\mathit{n},\;\mathit{bs})}(\sigma)}$$ where $n \le k$ indicates how many blocks we want to rollback, $\mathit{bs}$ is a list of new blocks we want to apply, and $\mathtt{length} \; \mathit{bs} \ge n$.
...
Intuitively, [\[time-stable-under-evolution\]](#time-stable-under-evolution){reference-type="ref+label" reference="time-stable-under-evolution"} says that we might not be able to do time conversion for some time $t$ because it's outside our current forecast range, but *if* it is within forecast range, then we don't need to earmark the answers we get from conversion as "subject to rollback": either we don't know, or we know for sure. This requirement may not be strictly *required* for consensus to operate ([\[future:relax-time-requirements\]](#future:relax-time-requirements){reference-type="ref+label" reference="future:relax-time-requirements"}), but it is a useful assumption which simplifies reasoning about time both within consensus and within clients of the consensus layer such as the wallet.
...
Many ledgers set the width of the safe zone such that it guarantees at least $k$ blocks, but *in principle* there is no need for the width of the safe zone to be related to $k$ at all, although other parts of consensus might have requirements for the width of the safe zone; we will discuss that in the next section ([1.4.2](#time:ledgerrestrictions:safezones){reference-type="ref+label" reference="time:ledgerrestrictions:safezones"}).

### Size of the safezones {#time:ledgerrestrictions:safezones}
...
As discussed in [\[forecast:ledgerview\]](#forecast:ledgerview){reference-type="ref+label" reference="forecast:ledgerview"}, it is important that the forecast range of the ledger to allow us to validate at least $k + 1$ blocks after the ledger tip; consequently, the safe zone of the ledger must be wide enough to guarantee that it can span $k + 1$ blocks. This combination of the requirements of the ledger with the header/body split ([\[nonfunctional:network:headerbody\]](#nonfunctional:network:headerbody){reference-type="ref+label" reference="nonfunctional:network:headerbody"}) means that in practice the width of the safe zone should be at least equal to the forecast range of the ledger, and hence defined in terms of $k$ after all.

### Stability should not be approximated

We discussed in [1.2](#time:slots-vs-blocks){reference-type="ref+label" reference="time:slots-vs-blocks"} that the ledger uses uses slot numbers to approximate stability. Such an approximation would violate [\[time-stable-under-evolution\]](#time-stable-under-evolution){reference-type="ref+label" reference="time-stable-under-evolution"}, however. Although we never switch to a shorter chain in terms of blocks, it is certainly possible that we might switch to a chain with a smaller *slot* number at its tip: this would happen whenever we switch to a longer but denser chain. If stability would be based on slot numbers, this might mean that we could go from a situation in which the era transition is considered known (and hence the forecast extends into the next era) to a situation in which the era transition is not yet considered known (and hence the forecast range only includes the safe zone in the current era).
...
[1.1](#hfc:era-transition-becoming-known){reference-type="ref+Label" reference="hfc:era-transition-becoming-known"} shows how the forecast range changes as the next era transition becomes known; as shown, the next era starts at the earliest possible moment (right after the safe zone); in general it could start later than that, but of course not earlier (that would violate the definition of the safe zone).

:::: {#hfc:era-transition-becoming-known .figure}
$$\begin{equation}
\begin{tikzpicture}[baseline=0pt]
...
### Cross-fork conversions {#time:cross-fork}
...
::: proof
*Proof.* Since $t$ is within the forecast range at $A$, by definition $\mathtt{Conv_{A}(t)}$ is defined. By monotonicity ([\[time-conversion-monotone\]](#time-conversion-monotone){reference-type="ref+label" reference="time-conversion-monotone"}) we must have $$\begin{align*}
...
## Avoiding time {#hfc:avoiding-time}
...
2.  When we add new blocks to the chain DB, we need to check if their slot number is ahead of the wallclock ([\[chainsel:infuture\]](#chainsel:infuture){reference-type="ref+label" reference="chainsel:infuture"}).
...
None of these use cases require either forecasting or cross-chain conversions. The most important example of where forecasting is required is in projecting the ledger view, as discussed in [1.4.2](#time:ledgerrestrictions:safezones){reference-type="ref+label" reference="time:ledgerrestrictions:safezones"}. Cross-fork conversions ([1.5.2](#time:cross-fork){reference-type="ref+label" reference="time:cross-fork"}) may arise for example when the consensus layer makes time conversions available to tooling such as the wallet, which may use it for example to show the wallclock of slots of blocks that may not necessarily live on the current chain.

Keeping track of era transitions, and providing time conversions that take them into account, is the responsibility of the hard fork combinator and we will discuss it in more detail in [\[hfc:time\]](#hfc:time){reference-type="ref+label" reference="hfc:time"}.
...
### "Header in future" check {#time:header-infuture-check}

Recall from [\[nonfunctional:network:headerbody\]](#nonfunctional:network:headerbody){reference-type="ref+label" reference="nonfunctional:network:headerbody"} that block downloading proceeds in two steps: first, the chain sync client downloads the block header and validates it; if it finds that the header is valid, the block download logic may decide to also download the block body, depending on chain selection ([\[consensus:overview:chainsel,consensus:class:chainsel\]](#consensus:overview:chainsel,consensus:class:chainsel){reference-type="ref+label" reference="consensus:overview:chainsel,consensus:class:chainsel"}).
...
As we have seen in [1.5.2](#time:cross-fork){reference-type="ref+label" reference="time:cross-fork"}, if $Q$ is within the *time* forecast range at $A$---put another way, if the time forecast range is at least as wide as the ledger forecast range---then we also can use the ledger state at $P$ to do time conversions at point $Q$. Moreover, as we saw in [1.4.2](#time:ledgerrestrictions:safezones){reference-type="ref+label" reference="time:ledgerrestrictions:safezones"}, for many ledgers that inclusion *must* hold. If we make this a requirement for *all* ledgers, in principle the chain sync client could do a header-in-future check.

For simplicity, however, we nonetheless omit the check. As we will see in the next section, the chain database must repeat this check *anyway*, and so doing it ahead of time in the chain sync client does not help very much; skipping it avoids one more use of time within the consensus layer. Indeed, a case could be made that we could skip header validation altogether, which would alleviate the need for forecasting *at all*; we will come back to this in [\[future:eliminating-forecasting\]](#future:eliminating-forecasting){reference-type="ref+label" reference="future:eliminating-forecasting"}.

### Ahead-of-time "block in future" check {#time:block-infuture-check}
...
It turns out that in general we cannot, not even in relatively common cases. Consider again the diagram from [1.6.1](#time:header-infuture-check){reference-type="ref+label" reference="time:header-infuture-check"}, but specialised to the typical case that the upstream node is on the same chain as we are, but a bit ahead of us:
...
### "Immutable tip in future" check {#time:imm-tip-in-future}

The chain database never adopts blocks from the future ([\[chainsel:infuture\]](#chainsel:infuture){reference-type="ref+label" reference="chainsel:infuture"}). Nevertheless, it is possible that if the user sets their computer system clock back by (the equivalent of) more than $k$ blocks, the immutable database ([\[storage:components\]](#storage:components){reference-type="ref+label" reference="storage:components"}) might contain blocks whose slot numbers are ahead of the wall clock. We cannot verify this during a regular integrity check of the immutable database because, as we have seen in this chapter, we would need a ledger state to do so, which we are not constructing during that integrity check. For now, we simply omit this check altogether, declaring it to be the user's responsibility instead to do a fresh install if they do reset their clock by this much.

However, in principle this check is not difficult: we initialise the immutable DB *without* doing the check, then initialise the ledger DB, passing it the immutable DB (which it needs to replay the most recent blocks, see [\[ledgerdb\]](#ledgerdb){reference-type="ref+label" reference="ledgerdb"}), and then ask the ledger DB for the ledger state corresponding to the tip of the immutable database. That ledger state will then allow us to do time conversions for any of the blocks in the immutable DB, trimming any blocks that are ahead of the wallclock.

### Scheduling actions for slot changes {#time:scheduling-actions}
...
When we are far behind, however, the priority is not to reduce latency, but rather to improve *throughput*: we want to catch up as quickly as we can with the chain, and aren't producing blocks anyway ([1.6.4](#time:scheduling-actions){reference-type="ref+label" reference="time:scheduling-actions"}).
