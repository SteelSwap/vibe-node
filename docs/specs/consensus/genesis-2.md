# Ouroboros Genesis {#genesis}

## Introduction

### Background: understanding the Longest Chain rule {#genesis:background:longest-chain}
...
### Nodes joining late {#genesis:background:joining-late}

When new nodes join the network (or rejoin after having been offline for a while), they don't have the advantage of having been online since the start of the system, and have no sufficiently long prefix of the honest chain available. As we saw at the end of [1.1.1](#genesis:background:longest-chain){reference-type="ref+label" reference="genesis:background:longest-chain"}, simply looking at chain length is insufficient to distinguish the honest chain from malicious chains: given enough time, an adversary can produce a chain that is longer than the honest chain:
...
### The Density rule {#genesis:background:density-rule}

It is therefore clear that we need a different chain selection rule so that nodes can (re)join, and the Ouroboros Genesis paper [@cryptoeprint:2018:378] proposes one, shown in [1.1](#genesis:maxvalid-bg){reference-type="ref+label" reference="genesis:maxvalid-bg"}. In this chapter we will work with a slightly simplified (though equivalent) form of this rule, which we will term the Density Rule:
...
(We will briefly discuss the differences between the rule in the paper and this one in [1.2.3](#genesis:original){reference-type="ref+label" reference="genesis:original"}.) Technically speaking, $s$ is a parameter of the rule, but the following default is a suitable choice both from a chain security perspective and from an implementation perspective:[^4]
...
Just to be very explicit, [\[lemma:tip-density-is-chain-length\]](#lemma:tip-density-is-chain-length){reference-type="ref+label" reference="lemma:tip-density-is-chain-length"} does *not* hold when the intersection is more than $s$ slots away:
...
[\[lemma:adversarial-before-k\]](#lemma:adversarial-before-k){reference-type="ref+Label" reference="lemma:adversarial-before-k"} has a useful specialisation for chains that an adversary might try to construct near the wallclock:
...
The intersection between the alert node's chain and the adversarial chain is within $s$ slots from the wallclock. This means that density at the intersection point is just chain length ([\[lemma:tip-density-is-chain-length\]](#lemma:tip-density-is-chain-length){reference-type="ref+label" reference="lemma:tip-density-is-chain-length"}), and hence the property follows from [\[lemma:adversarial-before-k\]](#lemma:adversarial-before-k){reference-type="ref+label" reference="lemma:adversarial-before-k"}. ◻
::::

### The original genesis rule {#genesis:original}

The Density rule is simplification of the rule as presented in the paper [@cryptoeprint:2018:378]. The original rule is shown in [1.1](#genesis:maxvalid-bg){reference-type="ref+label" reference="genesis:maxvalid-bg"}, and paraphrased below:
...
As we saw in [\[lemma:rule-equivalence\]](#lemma:rule-equivalence){reference-type="ref+label" reference="lemma:rule-equivalence"}, the Density rule is equivalent to the Longest Chain rule if the intersection is within $k$ blocks, so the original rule and the simplified form are in fact equivalent.
...
**Parameters**\
  ...
  $k$                                    Security parameter ([\[consensus:overview:k\]](#consensus:overview:k){reference-type="ref+label" reference="consensus:overview:k"})
  $s$                                    Genesis window size (Genesis rule specific parameter)
  $f$                                    Active slot coefficient ([\[praos:f\]](#praos:f){reference-type="ref+label" reference="praos:f"})
  -------------------------------------- -------------------------------------------------------------------------------------------------------------------------------------
...
[\[lemma:fragment-selection\]](#lemma:fragment-selection){reference-type="ref+Label" reference="lemma:fragment-selection"} is relevant because it reflects how the consensus layer uses chain selection:

1.  We maintain a fragment of the chain for each upstream peer we track ([\[chainsyncclient\]](#chainsyncclient){reference-type="ref+label" reference="chainsyncclient"}). The block fetch client ([\[chainsyncclient:plausiblecandidates\]](#chainsyncclient:plausiblecandidates){reference-type="ref+label" reference="chainsyncclient:plausiblecandidates"}) then picks a preferred fragment and downloads that.

2.  When the chain database needs to construct the current chain ([\[chainsel\]](#chainsel){reference-type="ref+label" reference="chainsel"}), it constructs a set of chain fragments through the volatile DB, all anchored at the tip of the immutable database, picks a preferred fragment, and adopts that as the node's current chain.

However, [\[lemma:fragment-selection\]](#lemma:fragment-selection){reference-type="ref+label" reference="lemma:fragment-selection"} is less useful than it might seem: the look-ahead closure requirement means that in the worst case, we still need to see entire chains before we can make a decision: every new intersection point requires us to see $s$ more slots:

::: center
:::

Moreover, due to the header/body split ([\[nonfunctional:network:headerbody\]](#nonfunctional:network:headerbody){reference-type="ref+label" reference="nonfunctional:network:headerbody"}), when we are tracking the headers from an upstream peer, we cannot (easily) verify headers that are more than $3k/f$ slots away from the intersection between our chain and their chain (see also [\[low-density\]](#low-density){reference-type="ref+label" reference="low-density"}). In the next section we will therefore consider how we can drop the look-ahead closure requirement.
...
## Prefix selection {#genesis:prefix-selection}
...
Prefix selection fits very naturally with the needs of the consensus layer (*cf.* the description of how consensus uses fragment selection in [1.3](#genesis:fragment-selection){reference-type="ref+label" reference="genesis:fragment-selection"}):
...
::: proof
*Proof (sketch).* The proof is very similar to the proof of [\[lemma:rule-equivalence\]](#lemma:rule-equivalence){reference-type="ref+label" reference="lemma:rule-equivalence"}. If the intersection is at most $k$ blocks away, it will be less than $s$ slots away; therefore prefix selection will be able to see the chains to their tip, the density of all chains will be known, and the densest fragment equals the longest one. ◻
:::
...
When the chain sync client is deciding which of the chains of its upstream peers to download, it does so based on chains of *headers*. It does not download block bodies and so cannot verify them. As such, it is basing its decisions based on header validity *only*. However, this is then only used to tell the block fetch client which blocks to *download* ([\[chainsyncclient:plausiblecandidates\]](#chainsyncclient:plausiblecandidates){reference-type="ref+label" reference="chainsyncclient:plausiblecandidates"}); it does not necessarily mean that those blocks will be *adopted*. When the chain database performs chain selection ([\[chainsel\]](#chainsel){reference-type="ref+label" reference="chainsel"}), it will verify the blocks and discard any that turn out to be invalid. If any blocks *are* invalid, then the chain sync client will disconnect from the nodes that provided them, which in turn may change which prefix is chosen by prefix selection.

Indeed, the only reason to even validate headers at all is to avoid a denial of service attack where an adversary might cause us to waste resources. It may be worth reconsidering this risk, and balancing it against the costs for the implementation; the only reason we need forecasting, and a special treatment of low density chains ([\[low-density\]](#low-density){reference-type="ref+label" reference="low-density"}) is that we insist we want to validate headers independent from blocks.

### Known density

[\[prefix-selection\]](#prefix-selection){reference-type="ref+Label" reference="prefix-selection"} requires known density at the anchor $a$ of the set, so that it can resolve the initial fork. This means we have to wait until we have downloaded enough headers from each peer: the last header we downloaded must either be outside the $s$ window, or else it must be the last header on that peer's chain (in which case the peer will tell us so). If a peer tells us they have more headers but then do not provide them, we should disconnect from them after some time-out to avoid a potential denial of service attack.
...
Prefix selection however will *wait*: it hasn't yet verified the header from $C$, and so it considers the density of $C$'s chain's to be unknown, and hence it cannot make a decision. *This is the correct behaviour*: the old implementation would select $C$'s chain immediately, but it would do so for a terrible reason: sure, it's longer, but only *because we downloaded more of that chain*; this is the circular reasoning we described above in [1.1.2](#genesis:background:joining-late){reference-type="ref+label" reference="genesis:background:joining-late"}.

In principle it would be possible for $D$ to *skip* the verification of any number of headers if those headers have already been verified from another peer. However, this would be optimising the code for the best case, with performance degrading when the peers are on different chains; this is something we avoid throughout ([\[nonfunctional:best-is-worst\]](#nonfunctional:best-is-worst){reference-type="ref+label" reference="nonfunctional:best-is-worst"}).

## Avoiding long rollbacks {#genesis:avoiding-long-rollbacks}

### Representative sample

At the end of [1.1.3](#genesis:background:density-rule){reference-type="ref+label" reference="genesis:background:density-rule"} we mentioned that if we have a situation such as
...
Such long rollbacks are problematic; we depend on having a maximum rollback of $k$ blocks in many ways ([\[consensus:overview:k,storage:components,chainsyncclient:validation,chainsyncclient:forecasting\]](#consensus:overview:k,storage:components,chainsyncclient:validation,chainsyncclient:forecasting){reference-type="ref+label" reference="consensus:overview:k,storage:components,chainsyncclient:validation,chainsyncclient:forecasting"} and others), and we will want to *continue* to depend on it. However, we needed this long rollback in this example only because we *first* adopted the adversary's chain, and *then* compared it to the honest chain. In the consensus layer even as it is today, we already don't do chain selection in such a chain-by-chain manner; as we saw in [1.3](#genesis:fragment-selection){reference-type="ref+label" reference="genesis:fragment-selection"} and again in [1.4](#genesis:prefix-selection){reference-type="ref+label" reference="genesis:prefix-selection"}, we instead pick the best out of all currently known candidates. This means that as long as we are *aware* of both chains before we adopt either one, we will just pick the honest chain straight away, and we avoid the rollback.
...
### Becoming alert {#genesis:becoming-alert}
...
[\[recover-alert-status\]](#recover-alert-status){reference-type="ref+Label" reference="recover-alert-status"} gives us a way to discover that we are up to date. Deciding when we are *not* up to date is less critical. One option is to simply use the inverse of [\[recover-alert-status\]](#recover-alert-status){reference-type="ref+label" reference="recover-alert-status"} and say we are not up to date when one of our peers provides us with a chain that we cannot see until its tip. Another option is to assume we are not up to date when we boot up the node, and then only conclude that have somehow fallen behind again if we notice that our tip is more than a certain distance away from the wallclock (at most $s$ slots). This may need some special care; if nodes stop producing blocks for a while, we might end up in a state in which we both conclude that we are up to date (because we can see all of our peer's chains to their tip) and not up to date (because our tip is too far from the wallclock). However, this scenario needs special case anyway; we will come back to it in [\[low-density\]](#low-density){reference-type="ref+label" reference="low-density"}.
...
### Chain selection in the chain database {#genesis:chain-database}

We mentioned in [1.4](#genesis:prefix-selection){reference-type="ref+label" reference="genesis:prefix-selection"} that the prefix selection interface works equally well for the chain sync client and the chain database. They are however not doing the same thing; this is a subtle point that deserves to be spelled out in detail.

It comes back to the difference between perceived chain length and actual chain length ([1.1.2](#genesis:background:joining-late){reference-type="ref+label" reference="genesis:background:joining-late"}). In the chain sync client this is a meaningful and useful difference: since we are tracking chains from individual peers, it makes sense to distinguish between having seen the tip of that particular chain, or only seeing a prefix of that chain. However, unless we completely change the API to the chain database, the chain database just sees individual blocks, without knowing anything about their provenance; it does therefore not know if those blocks are the tip of "their" chains; it's not even clear what that would mean.
...
This is not necessarily problematic: since the chain sync client has more context, it will make the right decision, and only present blocks from chain $A$ to the database. Indeed, as we saw in [1.5.2](#genesis:becoming-alert){reference-type="ref+label" reference="genesis:becoming-alert"}, we will in fact download *only* blocks from the honest chain until we are $s$ slots away from the wallclock, at which point we do one final chain selection, and we are up to date. At this point the Density Rule is *anyway* just selecting the longest chain, so the fact that the chain database is effectively doing longest chain selection *always* does not matter.
...
There is one advantage to this. [1.4](#genesis:prefix-selection){reference-type="ref+Label" reference="genesis:prefix-selection"} describes how the chain database could *in principle* use prefix selection: compute all paths through the volatile database and then use prefix selection to construct a prefix that the node should adopt as its current chain. While this is a useful *specification* of what the chain database must do, in practice we will probably need an equivalent of [\[focusonnewblock\]](#focusonnewblock){reference-type="ref+label" reference="focusonnewblock"} that will allow us to avoid looking at the entire volatile database every time a new block is added to the database. If we however decide that the chain database is just selecting based on chain length *anyway*, then the existing lemma and existing implementation can be used as-is.

### Abstract chain selection interface

The current chain selection API compares two chains at a time, and only looks at the tips of those two chains ([\[consensus:overview:chainsel\]](#consensus:overview:chainsel){reference-type="ref+label" reference="consensus:overview:chainsel"}). This will obviously have to change; depending on how exactly we want to modify the chain database ([1.6.1](#genesis:chain-database){reference-type="ref+label" reference="genesis:chain-database"}), we must either replace the existing interface with prefix selection as the primitive operation, or else add prefix selection as an additional concept.
...
### Possible optimisations {#genesis:optimisations}
...
- We saw in [1.5.2](#genesis:becoming-alert){reference-type="ref+label" reference="genesis:becoming-alert"} that any blocks that we download while syncing which are more than $s$ slots away from the wall clock, will be blocks from the common prefix of the honest chains and will not have to be rolled back. It might therefore be possible to bypass the volatile database entirely. However, how this works when we switch back from being up to date to not being up to date would require careful thought.
