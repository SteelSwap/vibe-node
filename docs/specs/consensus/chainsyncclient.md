...
NOTE: Currently low-density contains such a discussion.

## Trimming
## Interface to the block fetch logic
We should discuss here the (very subtle!) reasoning about how we establish the precondition that allows us to compare candidates (chainsel:fragments:precondition). See `plausibleCandidateChain` in `NodeKernel` (PR #2735).
