...
$$\begin{array}{lrclr}
    ...
    \textrm{Stack} & s      & ::= & f^*\\
    \textrm{CEK value} & V &  ::= & \mathsf{VCon}{\tn}{c} \enspace | \enspace \mathsf{VDelay}{M}{\rho}
       \enspace| \enspace \mathsf{VLamAbs}{x}{M}{\rho} \enspace \\
       &&& | \enspace \mathsf{VConstr}{i}{\overline{V}} \enspace | \enspace \mathsf{VBuiltin}{b}{\overline{V}}{\eta}\\
...
$$\begin{array}{lrclr}
        \textrm{Frame} & f  & ::=   & \inForceFrame                                                       & \textrm{force}\\
                       ...
                       &    &       & \inConstrFrame{i}{\overline{V}}{(\overline{M}, \rho)}           & \textrm{constructor argument}\\
                       &    &       & \inCaseFrame{(\overline{M}, \rho)}                                & \textrm{case scrutinee}
...
To make the CEK machine fit into the built-in evaluation mechanism defined in Section sec:specify-builtins we define $\mathsf{Inputs} = V$ and $\mathsf{Con}{\tn} =
\{\mathsf{VCon}{\tn}{c} : \tn \in \mathsf{Uni}, c \in \lbrack\!\lbrack \tn \rbrack\!\rbrack\}$.

The rules in Figure 5 show the transitions of the machine; if any situation arises which is not included in these transitions (for example, if a frame $\inAppRightFrame{\mathsf{VCon}{\tn}{c}}$ is encountered or if an attempt is made to apply `force` to a partial builtin application which is expecting a term argument), then the machine stops immediately in an error state.
...
{2}
 ...
 s;\rho & \compute \con{\tn}{c}                       &~\mapsto~& s \return \mathsf{VCon}{\tn}{c}\\
 s;\rho & \compute \lamU{x}{M}                       &~\mapsto~& s \return \mathsf{VLamAbs}{x}{M}{\rho}\\
 s;\rho & \compute \delay{M}                         &~\mapsto~& s\return \mathsf{VDelay}{M}{\rho}\\
 ...
 s;\rho & \compute \appU{M}{N}                       &~\mapsto~& \inAppLeftFrame{(N,\rho)} \cons s ;\rho \compute M\\
  s;\rho & \compute \constr{i}{M \cons \overline{M}} &~\mapsto~& \inConstrFrame{i}{[]}{(\overline{M},\rho)} \cons s ;\rho \compute M\\
 s;\rho & \compute \constr{i}{[]} &~\mapsto~&  s \return \mathsf{VConstr}{i}{[]}\\
 s;\rho & \compute \kase{N}{\overline{M}} &~\mapsto~&  \inCaseFrame{(\overline{M},\rho)} \cons s ;\rho \compute N\\
% No nullary builtins (yet)
 s;\rho & \compute \builtin{b}                      &~\mapsto~& s \return \mathsf{VBuiltin}{b}{[]}{\arity{b}}\\
...
\inAppRightFrame{\mathsf{VLamAbs}{x}{M}{\rho}} \cons s   & \return V  &~\mapsto~& s;\rho[x \mapsto V] \compute M\\
\inAppLeftFrame{V} \cons s   & \return \mathsf{VLamAbs}{x}{M}{\rho}  &~\mapsto~& s;\rho[x \mapsto V] \compute M\\
\inAppRightFrame{\mathsf{VBuiltin}{b}{\overline{V}}{(\iota \cons \eta)}} \cons s & \return V &~\mapsto~&
                         s \return \mathsf{VBuiltin}{b}{(\overline{V} \snoc V)}{\eta} \enskip \text{if $\iota \in \mathsf{Unihash} \cup \mathsf{VarStar}$}\\
\inAppLeftFrame{V} \cons s & \return \mathsf{VBuiltin}{b}{\overline{V}}{(\iota \cons \eta)} &~\mapsto~&
                         s \return \mathsf{VBuiltin}{b}{(\overline{V} \snoc V)}{\eta} \enskip \text{if $\iota \in \mathsf{Unihash} \cup \mathsf{VarStar}$}\\
\inAppRightFrame{\mathsf{VBuiltin}{b}{\overline{V}}{[\iota]}} \cons s  & \return V &~\mapsto~&
                         \mathsf{EvalCEK}\,(s, b, \overline{V}\snoc V) \enskip \text{if $\iota \in \mathsf{Unihash} \cup \mathsf{VarStar}$}\\
\inAppLeftFrame{V} \cons s & \return \mathsf{VBuiltin}{b}{\overline{V}}{[\iota]} &~\mapsto~&
                         \mathsf{EvalCEK}\,(s, b, \overline{V}\snoc V) \enskip \text{if $\iota \in \mathsf{Unihash} \cup \mathsf{VarStar}$}\\
\inForceFrame{} \cons s & \return \mathsf{VDelay}{M}{\rho}         &~\mapsto~& s;\rho \compute M\\
\inForceFrame{} \cons s & \return \mathsf{VBuiltin}{b}{\overline{V}}{(\iota \cons \eta)} &~\mapsto~&
                         s \return \mathsf{VBuiltin}{b}{\overline{V}}{\eta} \enskip \text{if $\iota \in \mathsf{QVar}$}\\
\inForceFrame{} \cons s & \return \mathsf{VBuiltin}{b}{\overline{V}}{[\iota]}   &~\mapsto~&
                         \mathsf{EvalCEK}\,(s, b, \overline{V}) \enskip \text{if $\iota \in \mathsf{QVar}$}\\
\inConstrFrame{i}{\overline{V}}{(M \cons \overline{M}, \rho)} \cons s & \return V   &~\mapsto~&
                         \inConstrFrame{i}{\overline{V} \cons V}{(\overline{M}, \rho)} \cons s;\rho \compute M \\
\inConstrFrame{i}{\overline{V}}{([], \rho)} \cons s & \return V   &~\mapsto~&
                         s \return \mathsf{VConstr}{i}{\overline{V} \cons V} \\
\inCaseFrame{(M_1 \ldots M_n, \rho)} \cons s & \return \mathsf{VConstr}{i}{V_1 \ldots V_m}   &~\mapsto~&
...
$$\begin{align*}
 \mathsf{EvalCEK}(s, b, [V_1, \ldots, V_n]) =
  &   \begin{cases}
      ...
      \end{cases}\\
  &  \;\;\text{where $r = \mathsf{Eval}\,(b,[V_1, \ldots, V_n])$}\\
\end{align*}$$
...
$$\begin{align*}
      \mathsf{Unload}{\rho}{M} &= [(\unload{V_1})/x_1]\cdots[(\unload{V_n})/x_n]M \quad
