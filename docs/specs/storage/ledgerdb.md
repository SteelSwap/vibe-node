...
2.  **Maintaining the past $k$ ledger states**: As discussed in consensus:overview:k, we might roll back up to $k$ blocks when switching to a more preferable fork. Consider the example below:
    ...
    - **Local state query server**: To query any of the past $k$ ledger states (servers:lsq).

    - **Chain sync client**: To validate headers of a chain that intersects with any of the past $k$ blocks (chainsyncclient:validation).
...
Note that whenever we say "ledger state", we mean the `ExtLedgerState blk` type described in storage:extledgerstate.
...
The `LedgerDB` is parameterised over the ledger state $l$. Conveniently, the `LedgerDB` can implement the same abstract interface (described in ledger:api) that the ledger state itself implements. I.e., the `GetTip`, `IsLedger`, and `ApplyBlock` classes. This means that in most places, wherever a ledger state can be used, it is also possible to wrap it in a `LedgerDB`, causing it to automatically maintain a history of the last $k$ ledger states.
...
## On-disk
The `LedgerDB.OnDisk` module provides functions to write a ledger state to disk and read a ledger state from disk. The `EncodeDisk` and `DecodeDisk` classes from serialisation:storage are used to (de)serialise the ledger state to or from CBOR. Because of its large size, we read and deserialise the snapshot incrementally.
...
### Initialisation
During initialisation, the goal is to construct an initial `LedgerDB` that is empty except for the ledger state at the anchor, which has to correspond to the immutable tip, i.e., the block at the tip of the Immutable DB (immutable).
