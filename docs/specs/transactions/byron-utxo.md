...
Functions on the types introduced in 4{reference-type="ref+label" reference="fig:rules:utxo"} are defined in 2{reference-type="ref+label" reference="fig:derived-defs:utxo"}. In particular, note that in function $\fun{minfee}$ we make use of the fact that the $\Lovelace$ type is an alias for the set of the integers numbers ($\mathbb{Z}$).
...
9{reference-type="ref+label" reference="fig:rules:utxow-seq"} models the application of a sequence of transactions. For the sake of concision we omit the types of this transition system, since they are the same as the ones of $\trans{utxow}{}$.
