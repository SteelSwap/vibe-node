...
## UTxO Transitions
Figure [1](#fig:functions:utxo) defines functions needed for the UTxO transition system. See Figure [\[fig:defs:utxo-shelley\]](#fig:defs:utxo-shelley) for most of the definitions used in the transition system.
...
- The calculation $\fun{consumed}$ gives the value consumed by the transaction $\var{tx}$ in the context of the protocol parameters, the current UTxO on the ledger and the registered stake credentials. This calculation is a sum of all coin in the inputs of $\var{tx}$, reward withdrawals and stake credential deposit refunds. Some of the definitions used in this function will be defined in Section [1.2](#sec:deps-refunds). In particular, $\fun{keyRefunds}$ is defined in Figure [4](#fig:functions:deposits-refunds).

- The calculation $\fun{produced}$ gives the value produced by the transaction $\var{tx}$ in the context of the protocol parameters and the registered stake pools. This calculation is a sum of all coin in the outputs of $\var{tx}$, the transaction fee and all needed deposits. Some of the definitions used in this function will be defined in Section [1.2](#sec:deps-refunds). In particular, $\fun{totalDeposits}$ is defined in Figure [4](#fig:functions:deposits-refunds).
...
The types for the UTxO transition are given in Figure [2](#fig:ts-types:utxo-shelley). The environment, $\UTxOEnv$, consists of:

- The current slot.

- The protocol parameters.

- The registered stake pools (also explained in Section [\[sec:delegation-shelley\]](#sec:delegation-shelley), Figure [\[fig:delegation-defs\]](#fig:delegation-defs)).

- The genesis key delegation mapping.

The current slot and registrations are needed for the refund calculations described in Section [1.2](#sec:deps-refunds).
...
- Proposed updates (see Section [\[sec:update\]](#sec:update)).
...
The UTxO transition system is given in Figure [3](#fig:rules:utxo-shelley). Rule [\[eq:utxo-inductive-shelley\]](#eq:utxo-inductive-shelley) specifies the conditions under which a transaction can be applied to a particular $\UTxOState$ in environment $\UTxOEnv$:
...
The accounting for the reward withdrawals is not done in this transition system. The rewards are tracked with the delegation state and will be removed in the final delegation transition, see  [\[eq:delegs-base\]](#eq:delegs-base).
...
Deposits are described in appendix B.2 of the delegation design document [@delegation_design]. These deposit functions were used above in the UTxO transition, [1.1](#sec:utxo-trans). Deposits are used for stake credential registration and pool registration certificates, which will be explained in Section [\[sec:delegation-shelley\]](#sec:delegation-shelley). In particular, the function $\cwitness{}$, which gets the certificate witness from a certificate, will be defined later. Figure [4](#fig:functions:deposits-refunds) defines the deposit and refund functions.
...
Figure [5](#fig:functions-witnesses) defines the function which gathers all the (hashes of) verification keys needed to witness a given transaction. This consists of:
...
The UTxOW transition system adds witnessing to the previous UTxO transition system. Figure [6](#fig:ts-types:utxow-shelley) defines the type for this transition.
...
Figure [7](#fig:rules:utxow-shelley) defines UTxOW transition. It has six predicates:
