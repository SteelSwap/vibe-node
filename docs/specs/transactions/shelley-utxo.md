...
Figure [1](#fig:functions:utxo){reference-type="ref" reference="fig:functions:utxo"} defines functions needed for the UTxO transition system. See Figure [\[fig:defs:utxo-shelley\]](#fig:defs:utxo-shelley){reference-type="ref" reference="fig:defs:utxo-shelley"} for most of the definitions used in the transition system.

- The function $\fun{outs}$ creates unspent outputs generated by a transaction, so that they can be added to the ledger state. For each output in the transaction, $\fun{outs}$ maps the transaction id and output index to the output.

- The $\fun{ubalance}$ function calculates sum total of all the coin in a given UTxO.

- The $\fun{wbalance}$ function calculates the total sum of all the reward withdrawals in a transaction.

- The calculation $\fun{consumed}$ gives the value consumed by the transaction $\var{tx}$ in the context of the protocol parameters, the current UTxO on the ledger and the registered stake credentials. This calculation is a sum of all coin in the inputs of $\var{tx}$, reward withdrawals and stake credential deposit refunds. Some of the definitions used in this function will be defined in Section [1.2](#sec:deps-refunds){reference-type="ref" reference="sec:deps-refunds"}. In particular, $\fun{keyRefunds}$ is defined in Figure [4](#fig:functions:deposits-refunds){reference-type="ref" reference="fig:functions:deposits-refunds"}.

- The calculation $\fun{produced}$ gives the value produced by the transaction $\var{tx}$ in the context of the protocol parameters and the registered stake pools. This calculation is a sum of all coin in the outputs of $\var{tx}$, the transaction fee and all needed deposits. Some of the definitions used in this function will be defined in Section [1.2](#sec:deps-refunds){reference-type="ref" reference="sec:deps-refunds"}. In particular, $\fun{totalDeposits}$ is defined in Figure [4](#fig:functions:deposits-refunds){reference-type="ref" reference="fig:functions:deposits-refunds"}.

For a transaction and a given ledger state, the preservation of value property holds exactly when the results of $\fun{consumed}$ equal the results of $\fun{produced}$. Moreover, when the property holds, value is only moved between transaction outputs, the reward accounts, the fee pot and the deposit pot.

Note that the $\fun{produced}$ function takes the registered stake pools ($\var{poolParams}$) as a parameter only in order to determine which pool registration certificates are new (and thus require a deposit) and which ones are updates. Registration will be discussed more in Section [\[sec:delegation-shelley\]](#sec:delegation-shelley){reference-type="ref" reference="sec:delegation-shelley"}.
...
The types for the UTxO transition are given in Figure [2](#fig:ts-types:utxo-shelley){reference-type="ref" reference="fig:ts-types:utxo-shelley"}. The environment, $\UTxOEnv$, consists of:

- The current slot.

- The protocol parameters.

- The registered stake pools (also explained in Section [\[sec:delegation-shelley\]](#sec:delegation-shelley){reference-type="ref" reference="sec:delegation-shelley"}, Figure [\[fig:delegation-defs\]](#fig:delegation-defs){reference-type="ref" reference="fig:delegation-defs"}).

- The genesis key delegation mapping.

The current slot and registrations are needed for the refund calculations described in Section [1.2](#sec:deps-refunds){reference-type="ref" reference="sec:deps-refunds"}.
...
- Proposed updates (see Section [\[sec:update\]](#sec:update){reference-type="ref" reference="sec:update"}).

The signal for the UTxO transition is a transaction.

:::: {#fig:ts-types:utxo-shelley .figure latex-placement="htb"}
...
The UTxO transition system is given in Figure [3](#fig:rules:utxo-shelley){reference-type="ref" reference="fig:rules:utxo-shelley"}. Rule [\[eq:utxo-inductive-shelley\]](#eq:utxo-inductive-shelley){reference-type="ref" reference="eq:utxo-inductive-shelley"} specifies the conditions under which a transaction can be applied to a particular $\UTxOState$ in environment $\UTxOEnv$:
...
- The coin value of each new output must be at least as large as the minimum value specified by the protocol parameter $\var{minUTxOValue}$.
...
- Update the UTxO:
  ...
  - Remove from the UTxO all the $(\var{txin}, \var{txout})$ pairs associated with the $\var{txins}$'s in the $\var{inputs}$ list of the transaction body $\var{txb}$.

  - Add all the $\var{outputs}$ of $\var{tx}$ to the UTxO, associated with the $\fun{txid}$ of the transaction body $\var{txb}$
...
The accounting for the reward withdrawals is not done in this transition system. The rewards are tracked with the delegation state and will be removed in the final delegation transition, see  [\[eq:delegs-base\]](#eq:delegs-base){reference-type="ref" reference="eq:delegs-base"}.
...
The approach of including refunds and rewards directly in the $outputs$ gives great flexibility to the management of the coin value obtained from these accounts, i.e. it can be directed to any address. However, it means there is no direct link between the $wdrls$ requests (similarly for the key deregistration certificate addresses and refund amounts) and the $outputs$. We verify that the included outputs are correct and authorized through the preservation of value condition and witnessing the transaction. The combination of the preservation of value and witnessing, described in Section [1.3](#sec:witnesses-shelley){reference-type="ref" reference="sec:witnesses-shelley"}, assures that the ledger state is updated correctly.

The main difference, however, in how rewards and refunds work is that refunds come from a $\var{deposits}$ pool, which is a single coin value indicating the total decayed amount of all the deposits ever made, while rewards come from individual accounts where a reward is accumulated to a specific address.
...
\label{eq:utxo-inductive-shelley}
    ...
    { \var{txb}\leteq\txbody{tx}
      ...
      \\
      \consumed{pp}{utxo}{rewards}~{txb} = \produced{pp}{poolParams}~{txb}
      \\
      ~
      \\
      {
...
## Deposits and Refunds {#sec:deps-refunds}

Deposits are described in appendix B.2 of the delegation design document [@delegation_design]. These deposit functions were used above in the UTxO transition, [1.1](#sec:utxo-trans){reference-type="ref" reference="sec:utxo-trans"}. Deposits are used for stake credential registration and pool registration certificates, which will be explained in Section [\[sec:delegation-shelley\]](#sec:delegation-shelley){reference-type="ref" reference="sec:delegation-shelley"}. In particular, the function $\cwitness{}$, which gets the certificate witness from a certificate, will be defined later. Figure [4](#fig:functions:deposits-refunds){reference-type="ref" reference="fig:functions:deposits-refunds"} defines the deposit and refund functions.

- The function $\fun{totalDeposits}$ returns the total deposits that have to be made by a transaction. This calculation uses two protocol parameters, namely the key deposit value and the pool deposit value. Note that those certificates which are updates of stake pool parameters of already registered pool keys should not (and are, in fact, not allowed to) make a deposit.

- The function $\fun{keyRefunds}$, calculates the total amount of returned deposits from stake key deregistration certificates.

  Note that $\fun{keyRefunds}$ uses the *current* protocol parameters. This means that any deposits made prior to a change in the deposit values will be refunded with the current value, not the one originally paid.
...
:::: {#fig:functions:deposits-refunds .figure latex-placement="htb"}
$$\begin{align*}
...
## Witnesses {#sec:witnesses-shelley}
...
Figure [5](#fig:functions-witnesses){reference-type="ref" reference="fig:functions-witnesses"} defines the function which gathers all the (hashes of) verification keys needed to witness a given transaction. This consists of:
...
The UTxOW transition system adds witnessing to the previous UTxO transition system. Figure [6](#fig:ts-types:utxow-shelley){reference-type="ref" reference="fig:ts-types:utxow-shelley"} defines the type for this transition.

:::: {#fig:ts-types:utxow-shelley .figure}
...
Figure [7](#fig:rules:utxow-shelley){reference-type="ref" reference="fig:rules:utxow-shelley"} defines UTxOW transition. It has six predicates:
...
:::: {#fig:rules:utxow-shelley .figure}
$$\begin{equation}
