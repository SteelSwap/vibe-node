...
def forge_loop(
    ...
    while not shutdown_event.is_set():
        ...
        # --- Atomic forge cycle ---
        # Acquire BOTH locks (chaindb then kernel) to prevent header
        # processing from changing nonce or tip mid-forge.
        # Lock order: chaindb → kernel (same as Thread 2 in
        # _locked_add_and_nonce). Prevents AB-BA deadlock.
        # Haskell ref: blockProcessed TMVar synchronization.
        _forged_this_slot = False
        try:
            with chain_db._lock.write() if chain_db is not None else _nullcontext():
                with node_kernel._lock.write() if node_kernel is not None else _nullcontext():
                    if node_kernel is not None:
